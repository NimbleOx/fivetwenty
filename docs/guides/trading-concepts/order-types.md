...
async def place_market_order() -> Any:
    ...
    async with AsyncClient() as client:
        print(f"Placing Market Order for Immediate Execution")
        ...
        print(f"   Execution: IMMEDIATE at market price")
        print(f"   Time-in-force: FOK (Fill or Kill)")
        ...
        try:
            ...
            print(f"\nMarket Order Execution Results:")
            print(f"   ID Order ID: {order_id}")
            print(f"   Submitted: {response.order_create_transaction.time}")
            ...
            if response.order_fill_transaction:
                ...
                print(f"   Status: PENDING (unusual for market orders)")
                print(f"   Note: Check market hours and liquidity")

        except Exception as e:
            print(f"\nError: Market order execution failed: {e}")
            print(f"Note: Common issues: Insufficient margin, market closed, invalid parameters")
            print(f"Recovery: Check account balance, verify market hours, adjust size")
            raise
...
async def place_limit_order() -> Any:
    ...
    async with AsyncClient() as client:
        print(f"Placing Limit Order for Price Discipline")
        ...
        print(f"   Patience: Wait for favorable market pricing")

        # Step 3: Explain limit order execution conditions
        print(f"\nExecution Conditions:")
        print(f"   WILL EXECUTE if market drops to {limit_price} or below")
        print(f"   Error: WILL NOT EXECUTE if market stays above {limit_price}")
        print(f"   PRICE GUARANTEE: Never pay more than {limit_price}")
        print(f"   TIME RISK: May never fill if price doesn't reach level")
        ...
        try:
            ...
            print(f"\nLimit Order Creation Results:")
            print(f"   ID Order ID: {order_id}")
            print(f"   Instrument Limit price: {order_price}")
            print(f"   Created: {order_time}")
            print(f"   Status: PENDING (waiting for market price)")
            print(f"   Time-in-force: GTC (active until filled/cancelled)")

            # Step 6: Provide limit order monitoring guidance
            print(f"\nOrder Monitoring Strategy:")
            print(f"   Watch Watch market: Monitor price movement toward {limit_price}")
            print(f"   Flexibility: Can modify or cancel if market conditions change")
            print(f"   Position Analysis: Review if limit level remains valid")
            print(f"   Patience: Allow time for market to reach your price")

            # Step 7: Market scenarios for limit order success
            print(f"\nFeature Favorable Market Scenarios:")
            print(f"   Bearish momentum: Price declining toward limit level")
            print(f"   Range trading: Price oscillating around limit level")
            print(f"   News News impact: Economic events pushing price to limit")
            print(f"   Support level: Limit placed at technical support")

            # Step 8: Risk considerations for limit orders
            print(f"\n⚠️ Limit Order Risk Considerations:")
            print(f"   Opportunity cost: May miss move if price doesn't reach limit")
            print(f"   Position Market gaps: Price may gap past limit (get better fill)")
            print(f"   Changing conditions: Market structure may invalidate level")
            print(f"   Time decay: Prolonged unfilled orders may become stale")

        except Exception as e:
            print(f"\nError: Limit order placement failed: {e}")
            print(f"Note: Common issues: Invalid price level, insufficient margin reservation")
            print(f"Recovery: Verify price format, check margin requirements")
            raise
...
async def place_stop_order() -> Any:
    ...
    async with AsyncClient() as client:
        print(f"Placing Stop Order for Risk Management")
        ...
        print(f"\nStop Order Risk Management Setup:")
        print(f"   Instrument Instrument: {instrument}")
        print(f"   Stop price: {stop_price}")
        print(f"   Position: {abs(position_size):,} units (SELL)")
        print(f"   Purpose: Risk protection or breakout strategy")
        print(f"   Trigger: Becomes market order when price hits {stop_price}")

        # Step 3: Explain stop order trigger mechanics
        print(f"\nStop Order Trigger Mechanics:")
        if position_size < 0:  # Sell stop
            print(f"   SELL STOP: Triggers when market falls to {stop_price}")
            print(f"   Use case: Stop-loss protection for long positions")
            print(f"   Note: Logic: Cut losses when market moves against you")
        else:  # Buy stop
            print(f"   BUY STOP: Triggers when market rises to {stop_price}")
            print(f"   Use case: Breakout entry for momentum strategies")
            print(f"   Note: Logic: Enter position when market breaks resistance")

        print(f"   Execution: Converts to MARKET ORDER when triggered")
        ...
        try:
            ...
            print(f"\nStop Order Creation Results:")
            print(f"   ID Order ID: {order_id}")
            print(f"   Trigger price: {trigger_price}")
            print(f"   Created: {order_time}")
            print(f"   Status: PENDING (monitoring market for trigger)")
            print(f"   Protection: Active until triggered or cancelled")

            # Step 6: Risk management implications
            print(f"\nRisk Management Benefits:")
            print(f"   Automatic: No manual intervention required")
            print(f"   24/7: Protection active during all market hours")
            print(f"   Discipline: Removes emotion from loss-cutting")
            print(f"   Instrument Capital preservation: Limits maximum loss exposure")

            # Step 7: Stop order execution scenarios
            print(f"\nExecution Scenarios:")
            print(f"   TRIGGERS when: Market price reaches {stop_price}")
            print(f"   BECOMES: Market order for immediate execution")
            ...
            print(f"   Adjustment: Move stops to protect profits")
            print(f"   ⚠️ Slippage: Account for execution price differences")
            print(f"   Position size: Ensure stop loss fits risk budget")
            ...
            print(f"   Volatile markets: Higher slippage risk")
            print(f"   News News events: Potential for large slippage")
            print(f"   Market gaps: Weekend/holiday gap risk")

        except Exception as e:
            print(f"\nError: Stop order placement failed: {e}")
            print(f"Note: Common issues: Invalid trigger price, margin requirements")
            print(f"Recovery: Check price level validity, verify account capacity")
            raise
...
async def place_mit_order() -> Any:
    ...
    async with AsyncClient() as client:
        print(f"Placing MIT Order for Profit Taking")
        ...
        print(f"\nMIT Order Profit-Taking Setup:")
        print(f"   Instrument Instrument: {instrument}")
        print(f"   MIT trigger: {mit_price}")
        ...
        print(f"   Execution: Market order when price touches {mit_price}")

        # Step 3: Explain MIT order logic vs Stop orders
        print(f"\nMIT vs Stop Order Logic:")
        print(f"   MIT Order: Triggers on FAVORABLE price movement")
        print(f"   Example: Take profit when price rises to target")
        print(f"   Stop Order: Triggers on UNFAVORABLE price movement")
        print(f"   Example: Cut losses when price falls below level")
        ...
        if position_size < 0:  # Closing long position
            print(f"   PROFIT TARGET: Sell when price rises to {mit_price}")
            print(f"   Instrument Logic: Close long position at profitable level")
            print(f"   Use case: Automated profit-taking for long trades")
        else:  # Opening short position
            print(f"   MEAN REVERSION: Buy when price rises to {mit_price}")
            print(f"   Logic: Enter counter-trend position at resistance")
            print(f"   Use case: Fade breakouts or sell high")
        ...
        try:
            ...
            print(f"\nMIT Order Creation Results:")
            print(f"   ID Order ID: {order_id}")
            print(f"   Trigger price: {trigger_price}")
            print(f"   Created: {order_time}")
            print(f"   Status: PENDING (waiting for profit opportunity)")
            ...
            print(f"   Discipline: Automatic profit capture at target")
            print(f"   Calm Emotion-free: Removes greed from profit-taking")
            print(f"   Optimization: Captures gains during favorable moves")
            print(f"   Availability: Works 24/7 without monitoring")

            # Step 8: MIT execution characteristics
            print(f"\nMIT Execution Characteristics:")
            print(f"   TRIGGERS when: Price touches {mit_price} favorably")
            print(f"   BECOMES: Market order for immediate execution")
            print(f"   Instrument FILLS at: Best available price near trigger")
            print(f"   ADVANTAGE: Captures profits on favorable moves")
            ...
            print(f"   Momentum: Strong moves may fill at worse prices")
            print(f"   Target level: May need adjustment as market evolves")
            print(f"   Opportunity cost: May miss larger moves beyond target")

            # Step 10: MIT order optimization strategies
            print(f"\nMIT Order Optimization:")
            ...
            print(f"   Dynamic adjustment: Move targets as trade develops")
            print(f"   Risk-reward: Ensure adequate profit vs potential loss")
            print(f"   Time frame: Match target to strategy timeframe")

        except Exception as e:
            print(f"\nError: MIT order placement failed: {e}")
            print(f"Note: Common issues: Invalid trigger level, position conflicts")
            print(f"Recovery: Verify trigger price, check existing positions")
            raise
...
print(f"Essential Order Parameters Configuration")
...
for param, value in order_params.items():
    if param == "instrument":
        ...
        print(f"   {param}: {value:,} ({direction} {abs(value):,} units)")
    elif param == "type":
        print(f"   {param}: {value} (Execution behavior)")
    elif param == "price":
        print(f"   {param}: {value} (Target execution price)")
    elif param == "time_in_force":
        print(f"   {param}: {value} (Order duration policy)")
...
print(f"\nUnits Parameter Logic:")
print(f"   Positive units: BUY order (go long)")
...
async def order_with_time_controls() -> Any:
    ...
    async with AsyncClient() as client:
        print(f"Time-In-Force Order Controls Demo")
        ...
        print(f"   Expiry time: {expiry_time.strftime('%Y-%m-%d %H:%M UTC')}")
        print(f"   Duration: 24 hours tactical window")
        print(f"   Strategy: Time-limited opportunity capture")
        ...
        print(f"   Time-in-force: GTD (expires automatically)")
        ...
        try:
            ...
            print(f"\nGTD Order Creation Results:")
            print(f"   ID Order ID: {order_id}")
            print(f"   Created: {order_time}")
            print(f"   Expires: {expiry_time}")
            print(f"   Status: PENDING (time-limited)")

            # Step 6: Time-in-force strategy implications
            print(f"\nNote: GTD Strategy Benefits:")
            print(f"   Auto-cleanup: Order expires without manual intervention")
            print(f"   Tactical: Perfect for event-driven trading")
            print(f"   Risk control: Prevents stale orders")
            print(f"   Discipline: Forces strategy re-evaluation")

            # Step 7: Alternative time-in-force demonstrations
            print(f"\nTime-In-Force Options Comparison:")
            ...
            print(f"\nTime-In-Force Use Cases:")
            print(f"   GTC: Long-term support/resistance levels")
            print(f"   GTD: Pre-earnings, pre-news event orders")
            print(f"   FOK: Large orders requiring complete fill")
            print(f"   IOC: Scalping, immediate partial execution")

            # Step 9: Time management best practices
            print(f"\nTime Management Best Practices:")
            print(f"   Position Regular review: Check pending orders daily")
            print(f"   Strategic expiry: Match duration to strategy")
            print(f"   Renewal decision: Evaluate before expiration")
            print(f"   Context awareness: Consider changing market conditions")

        except Exception as e:
            print(f"\nError: GTD order placement failed: {e}")
            print(f"Note: Common issues: Invalid expiry time, timezone problems")
            print(f"Recovery: Check time format, verify future timestamp")
            raise
    ...
    async with AsyncClient() as client:
        ...
        try:
            ...
            if order_state == "PENDING":
                ...
                print(f"   Target price: {order.price}")
                ...
                print(f"   Action: Wait for price {order.price} or manual intervention")
                ...
                print(f"   Status: Successfully executed")
                ...
                if hasattr(order, 'cancelling_transaction') and order.cancelling_transaction:
                    ...
                    print(f"   Note: Cancel reason: {cancel_reason}")
                    ...
                    if "CLIENT_REQUEST" in cancel_reason:
                        ...
                        print(f"   Expired due to GTD time limit")
            ...
            if order_state == "PENDING":
                ...
                print(f"   Review: Validate order still aligns with strategy")
                print(f"   Timeline: Consider time remaining for execution")
            elif order_state == "FILLED":
                print(f"   Position: Monitor resulting position performance")
                print(f"   Risk: Ensure stop losses are in place")
                print(f"   Position Management: Consider profit-taking levels")
            elif order_state in ["CANCELLED", "REJECTED"]:
                print(f"   Analysis: Review why order didn't execute")
                print(f"   Strategy: Adjust approach if needed")
                print(f"   Reentry: Place new order with updated parameters")

        except Exception as e:
            print(f"\nError: Order status retrieval failed: {e}")
            print(f"Note: Common issues: Invalid order ID, network problems")
            print(f"Recovery: Verify order ID, check connection")
            raise
...
async def modify_pending_order(order_id: str, new_price: Decimal) -> Any:
    ...
    async with AsyncClient() as client:
        print(f"Dynamic Order Modification System")
        print(f"   ID Order ID: {order_id}")
        ...
        try:
            ...
            print(f"   State: {current_order.state}")
            print(f"   Instrument: {current_order.instrument}")
            ...
            if current_order.state != "PENDING":
                ...
                print(f"   Order state: {current_order.state}")
            ...
            if response.order_create_transaction:
                ...
                print(f"\nOrder Modification Results:")
                print(f"   ID New order ID: {new_order_id}")
                print(f"   Instrument Updated price: {new_price}")
                print(f"   Modified: {modification_time}")
                print(f"   Status: PENDING (with new parameters)")

            # Step 7: Strategic modification guidance
            print(f"\nNote: Order Modification Best Practices:")
            print(f"   Market adaptation: Adjust to changing conditions")
            print(f"   Timing: Modify before market moves away")
            print(f"   Strategy alignment: Ensure modification fits plan")
            print(f"   Frequency: Avoid excessive modifications")

            # Step 8: Common modification scenarios
            print(f"\nCommon Modification Scenarios:")
            print(f"   Better entry: Lower limit price for better value")
            print(f"   Market pursuit: Raise price to catch moving market")
            print(f"   Technical update: Adjust to new support/resistance")
            print(f"   Time pressure: Modify before GTD expiration")
            ...
            print(f"   Timing: Brief window where order is inactive")
            print(f"   Strategy drift: Too many changes may blur plan")
            print(f"   Execution risk: Modified order may fill immediately")
            ...
            print(f"   Wait: Sometimes best to stick with original plan")

        except Exception as e:
            print(f"\nError: Order modification failed: {e}")
            print(f"Note: Common issues: Order already filled/cancelled, invalid price")
            print(f"Recovery: Check order status, validate new parameters")
            raise
...
async def robust_order_placement() -> Optional[Any]:
    ...
    async with AsyncClient() as client:
        print(f"Robust Order Placement with Error Handling")
        ...
        while retry_count < max_retries:
            try:
                print(f"\nOrder Placement Attempt #{retry_count + 1}")
                ...
                if "INSUFFICIENT_MARGIN" in error_message:
                    ...
                    print(f"   Recovery: Reducing position size by 50%")

                    # Reduce position size for retry
                    order_config["units"] = int(order_config["units"] * 0.5)
                    print(f"   New position size: {order_config['units']:,} units")
                    ...
                    print(f"   Recovery: Adjusting price toward market")
                    ...
                    print(f"\nINVALID_INSTRUMENT Error Handling:")
        ...
        print(f"   Recommendation: Review account status and market conditions")

        # Step 9: Error handling best practices summary
        print(f"\nNote: Error Handling Best Practices:")
        print(f"   Retry logic: Implement intelligent retry strategies")
        print(f"   Parameter adjustment: Adapt to error conditions")
        print(f"   Exponential backoff: Avoid overwhelming API")
        print(f"   Position Logging: Record all errors for analysis")
        print(f"   Graceful degradation: Fail safely when unrecoverable")
        print(f"   Monitoring: Alert on persistent failures")
...
def validate_order_params(instrument: str, units: int, price: Decimal) -> Any:
    ...
    print(f"   Valid: {decimal_places} decimal places (within {max_precision} limit)")

    # Step 3: Instrument format validation
    print(f"\nInstrument Format Validation:")
    print(f"   Provided instrument: {instrument}")
    ...
    print(f"   Valid: {base_currency}/{quote_currency} format")
...
async def calculate_position_size(risk_amount: Decimal, stop_distance: Decimal, instrument: str = "EUR_USD") -> Dict[str, Any]:
    ...
    print(f"Position Size Calculation System")
    print(f"   Instrument Risk amount: ${risk_amount}")
    print(f"   Stop distance: {stop_distance} price units")
    ...
    print(f"   Stop in price units: {stop_distance}")
    print(f"   Stop in pips: {stop_distance_pips:.1f} pips")
    ...
    print(f"   In lots: {position_size_units / 10000:.2f} mini lots")
    ...
    print(f"   Calculated: {position_size_units:.0f} units")
    print(f"   ⚠️ Absolute max: {max_position_absolute:,} units")
    print(f"   Conservative max: {max_position_conservative:,} units")
    print(f"   Final size: {constrained_size:,} units")
    ...
    if constrained_size < minimum_position:
        ...
        print(f"   Calculated size too small: {constrained_size} units")
        print(f"   Minimum viable: {minimum_position} units")
    ...
    print(f"   Target risk: ${risk_amount}")
    ...
    if risk_utilization > 100:
        ...
        print(f"   Near maximum: Consider slight reduction")
    elif risk_utilization > 70:
        print(f"   Well-utilized: Good risk management")
    else:
        print(f"   Conservative: Could increase if desired")
