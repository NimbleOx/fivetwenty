...
config = StreamingConfiguration(
    ...
    max_reconnect_attempts=10  # Try reconnecting up to 10 times
)
...
async def production_trading_system():
    """Production-ready streaming system with comprehensive error handling."""

    while True:  # Outer loop for reconnection
        try:
            async with AsyncClient(token=token, environment=Environment.LIVE) as client:
