...
### Addendum — found by the follow-up test-hardening pass (commit G)

- **F-SDK-15 · P1 · `_request` retried writes after timeouts.** The `TimeoutException` branch ignored the safe-methods gate, so a timed-out POST/PUT/PATCH was re-sent — a timed-out market order that actually reached OANDA could be double-submitted, contradicting the documented "only retry safe operations" policy. *Fixed: timeout retries now gated on `allow_retry`; regression test sends a POST through a timing-out transport and asserts exactly one attempt.*
- **F-SDK-16 · P2 · `_stream_with_retries` never yielded CONNECTING/RECONNECTING.** State was promoted to CONNECTED before every yield, so the SDK's own documented example (`if state == StreamState.RECONNECTING: ...`) could never trigger. *Fixed: the first line after a (re)connection carries the transition state; subsequent lines are CONNECTED.*
- **F-SDK-17 · P2 · Sync `stream_iter` silently dropped the newest event on backpressure.** On `queue.Full` the event that overflowed was discarded (despite the comment promising to drop old events), so slow consumers lost the freshest prices. *Fixed: the oldest queued event is evicted and the new one enqueued; deterministic regression test with a 2-slot queue.*
- **F-SDK-18 · P2 · `ApiResponse` snake_case attribute access missed `*_ids` and `*_vwap` fields.** `_snake_to_oanda` mapped `ids → IDS` (OANDA uses `IDs`) and had no `VWAP` entry, so `resp.trade_ids` / `resp.full_vwap` failed to resolve `tradeIDs` / `fullVWAP`. Caught by the new alias-consistency test, which now pins all ~950 hand-typed aliases against the converter (irregulars explicitly allowlisted). *Fixed in the converter.*
- **F-SDK-19 · P2 · `ConfigValidator.validate_account_id` rejected real OANDA account IDs.** The regex required exactly 7 digits in the user segment, but real accounts vary (this project's own practice account has 8 digits; repo fixtures use 6). *Fixed: only the site-division-user-account shape is enforced.*
- **P3 notes (documented, not changed):** the stall timer is only evaluated on keep-alive lines (mid-read hangs are bounded by the HTTP read timeout) — docstring now says so; `ReconnectionPolicy.max_attempts` counts reconnects beyond the initial connection — field description clarified. Harness quirks recorded by the new harness tests: `_normalize_default` float-coerces version-like strings; `assemble_domain_report` couples `REPORTS_DIR` to `REPO_ROOT`.
