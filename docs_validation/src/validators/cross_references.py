            ...
            try:
                ...
                if not target_path.exists():
                    issues.append(ValidationIssue(
                        message=f"Relative link target not found: {url}",
                        file_path=file_info.path,
                        line=line_num,
                        severity=IssueSeverity.ERROR,
                        rule_id="cross_ref_broken_link",
                        context=match.group(0),
                        suggestion=f"Check if the file exists at {target_path}"
                    ))

            except Exception as e:
                # Invalid path
                issues.append(ValidationIssue(
                    message=f"Invalid relative link path: {url} ({e})",
                    file_path=file_info.path,
                    line=line_num,
                    severity=IssueSeverity.WARNING,
                    rule_id="cross_ref_invalid_path",
                    context=match.group(0),
                    suggestion="Check the link path syntax"
                ))
            ...
            try:
                ...
                if not found_anchor:
                    issues.append(ValidationIssue(
                        ...
                        line=line_num,
                        severity=IssueSeverity.WARNING,
                        rule_id="cross_ref_broken_anchor",
                        context=match.group(0),
                        suggestion=f"Check if a header exists that would generate the anchor #{anchor}"
                    ))

            except Exception as e:
                # Couldn't read file or other error
                issues.append(ValidationIssue(
                    message=f"Could not validate anchor link #{anchor}: {e}",
                    file_path=file_info.path,
                    line=line_num,
                    severity=IssueSeverity.INFO,
                    rule_id="cross_ref_anchor_check_failed",
                    context=match.group(0),
                    suggestion="Manually verify the anchor link"
                ))
    ...
    def _check_reference_links(self, line: str, line_num: int, file_info: FileInfo) -> list[ValidationIssue]:
        ...
        for match in matches:
            ...
            if not ref_to_check.strip():
                issues.append(ValidationIssue(
                    message="Reference link has empty reference",
                    file_path=file_info.path,
                    line=line_num,
                    severity=IssueSeverity.WARNING,
                    rule_id="cross_ref_empty_reference",
                    context=match.group(0),
                    suggestion="Add a reference name or define the reference elsewhere"
                ))
    ...
    def _find_project_root(self, file_path: Path) -> Path:
        ...
        while current != current.parent:
            ...
            if any((current / indicator).exists() for indicator in [
                'pyproject.toml', 'setup.py', '.git', 'package.json', 'Cargo.toml'
            ]):
                return current
