            ...
            if any("float" in line and ("price" in line.lower() or "amount" in line.lower()) for line in lines):
                if not any("decimal" in line.lower() for line in lines):
                    issues.append(ValidationIssue(
                        message="File uses float for financial values but doesn't import Decimal",
                        file_path=file_info.path,
                        severity=IssueSeverity.ERROR,
                        rule_id="financial_precision_import",
                        suggestion="Add 'from decimal import Decimal' and use Decimal instead of float"
                    ))
        ...
        for pattern in financial_float_patterns:
            if re.search(pattern, line, re.IGNORECASE):
                issues.append(ValidationIssue(
                    ...
                    line=line_num,
                    severity=IssueSeverity.ERROR,
                    rule_id="financial_precision_float",
                    context=line.strip(),
                    suggestion="Replace float with Decimal from the decimal module"
                ))
