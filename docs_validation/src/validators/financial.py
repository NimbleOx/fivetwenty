...
class FinancialPrecisionValidator(BaseValidator):
    ...
    def _check_float_usage(self, line: str, line_num: int, file_path: Path) -> list[ValidationIssue]:
        ...
        issues.extend([
            ValidationIssue(
                message="Use Decimal instead of float for financial calculations", file_path=file_path, line=line_num, severity=IssueSeverity.ERROR, rule_id="financial_precision_float", context=line.strip(), suggestion="Replace float with Decimal from the decimal module"
            )
