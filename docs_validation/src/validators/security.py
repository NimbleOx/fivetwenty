...
class SecurityValidator(BaseValidator):
                    ...
                    if self._is_likely_real_secret(match.group(), line):
                        ...
                        if severity:
                            issues.append(ValidationIssue(
                                ...
                                line=line_num,
                                severity=severity,
                                rule_id="security_exposed_secret",
                                context=self._mask_secret(line, match.start(), match.end()),
                                suggestion="Remove or replace with placeholder value"
                            ))
    ...
    def _is_likely_real_secret(self, matched_text: str, line: str) -> bool:
        ...
        common_placeholders = {
            "your-api-token", "your-token", "api-token", "secret-key",
            "example-token", "demo-token", "test-token", "sample-token"
        }
