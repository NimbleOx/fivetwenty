#!/usr/bin/env python3
"""
Basic usage example for FiveTwenty.
...
async def main() -> None:
    ...
    async with AsyncClient(token=token, environment=Environment.PRACTICE) as client:
        try:
            ...
            # Get account details
            print("\n📊 Account details...")
            details = await client.accounts.get_account(account.id)
            ...
            instruments = await client.accounts.get_account_instruments(account.id, instruments=["EUR_USD", "GBP_USD", "USD_JPY"])
            ...
            # Get current prices
            print("\n💰 Current prices...")
            pricing = await client.pricing.get_pricing(account.id, ["EUR_USD", "GBP_USD"])
            ...
            async for event in client.pricing.get_pricing_stream(account.id, ["EUR_USD"]):
                if hasattr(event, "instrument"):  # Price update
                    # Type narrowing - we know it's a ClientPrice if it has 'instrument'
                    price = event  # This is a ClientPrice
                    if hasattr(price, "closeout_bid") and hasattr(price, "closeout_ask"):
                        spread = float(price.closeout_ask) - float(price.closeout_bid)
                        print(f"   {price.instrument}: {price.closeout_bid}/{price.closeout_ask} (spread: {spread:.5f})")
                    count += 1
