...
class TestAsyncClientConfiguration:
    ...
    async def test_direct_parameters(self):
        ...
        async with AsyncClient(token="test-token", account_id="test-account-id", environment=Environment.PRACTICE) as client:
    ...
    async def test_environment_variables_success(self):
    ...
    async def test_config_object_priority_over_params(self):
        """Test that config object takes priority over direct parameters."""
        config = AccountConfig(token="config-token", account_id="config-account-id", environment=Environment.LIVE, alias="config_account")
        async with AsyncClient(token="direct-token", environment=Environment.PRACTICE, config=config) as client:
            assert client._token == "config-token"
            assert client.account_id == "config-account-id"
            assert client._environment == Environment.LIVE
            assert client.config.alias == "config_account"

    async def test_direct_params_priority_over_env_vars(self):
        """Test that direct parameters take priority over env vars."""
        with patch.dict(os.environ, {"FIVETWENTY_OANDA_TOKEN": "env-token", "FIVETWENTY_OANDA_ACCOUNT": "env-account-id", "FIVETWENTY_OANDA_ENVIRONMENT": "live"}, clear=True):
            async with AsyncClient(token="direct-token", account_id="direct-account-id", environment=Environment.PRACTICE) as client:
                assert client._token == "direct-token"
                assert client.account_id == "direct-account-id"
                assert client._environment == Environment.PRACTICE

    def test_required_account_id(self):
        """Test that account_id is required for direct parameters."""
        with pytest.raises(ValueError, match="account_id is required when providing token directly"):
            AsyncClient(token="test-token", environment=Environment.PRACTICE)

    async def test_account_id_override(self):
        ...
        async with AsyncClient(config=config, account_id="override-account-id") as client:
            assert client._token == "config-token"
            assert client.account_id == "override-account-id"
            assert client._environment == Environment.PRACTICE
...
class TestClientConfiguration:
    ...
    def test_direct_parameters(self):
        ...
        with Client(token="test-token", account_id="test-account-id", environment=Environment.PRACTICE) as client:
            assert client.account_id == "test-account-id"
            assert client.config is not None
            assert client.config.alias == "direct_params"
...
class TestClientConfigurationEdgeCases:
    ...
    def test_invalid_environment_variable_value(self):
        ...
        with patch.dict(os.environ, {"FIVETWENTY_OANDA_TOKEN": "test-token", "FIVETWENTY_OANDA_ACCOUNT": "test-account-id", "FIVETWENTY_OANDA_ENVIRONMENT": "invalid"}, clear=True), pytest.raises(ValueError):
            AsyncClient()
    ...
    def test_empty_token_in_config(self):
        ...
        with pytest.raises(ValidationError):
            AccountConfig(token="   ", account_id="test-account-id", environment=Environment.PRACTICE, alias="test_account")

    def test_none_values_handled_properly(self):
        """Test that None values are handled properly."""
        with pytest.raises(ValueError, match="account_id is required when providing token directly"):
            AsyncClient(token="test-token", account_id=None, environment=Environment.PRACTICE)

    async def test_secret_values_not_logged(self):
