...
def test_api_response_missing_values_use_mapping_semantics() -> None:
    """Missing keys raise KeyError, missing attributes raise AttributeError."""
    response = ApiResponse({"lastTransactionID": "456"})

    assert response.get("missing", "fallback") == "fallback"
    assert "missing" not in response
    assert 42 not in response
...
def test_api_response_nested_model_miss_does_not_hide_key_errors() -> None:
    """Nested model fallback still raises KeyError when the model lacks the field."""
    trade = Trade(
        id="123",
        instrument="EUR_USD",
        price="1.1000",
